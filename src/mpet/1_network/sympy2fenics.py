...
Examples:
    ...
    Matrix([[1.0*cos(x), 0], [0, 1.0*cos(y)]])
    >>> sf.sympy2expr(w)                       # to FEniCS expression string
    (('x[0]', 'x[1]'), ('x[0]', 'x[2]'))
"""
...
def sympy2expr(expr):
    ...
    Examples:
        ...
        >>> sympy2expr(str2sympy('sin(x)*sin(y)'))
        'sin(x[0])*sin(x[1])'
    """
    ...
    if hasattr(expr, '__getitem__'):
        if expr.shape[0] == 1 or expr.shape[1] == 1:
            # Vector
            return tuple(map(to_ccode, expr))
        else:
            # Matrix
            return tuple([tuple(map(to_ccode, expr[i, :]))
                          for i in range(expr.shape[1])])
    else:
        # Scalar
        return to_ccode(expr)
...
def div(u):
    ...
    if u.shape[1] == 1 and len(u.shape) == 2:
        #print "column vector for:", u
        # Column vector
        return vec_div(u)
    elif u.shape[0] == 1 and len(u.shape) == 2:
        # Row vector
        #print "row vector for:", u
